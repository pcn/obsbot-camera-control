...
#pragma pack(1)
  ...
  public:
    ...
    int32_t info_type;
    std::string dev_sn;
    std::string devPermission;
    int devIsSupportCheck;
    int devIsPublicAccess;
...
#pragma pack()
  ...
  typedef std::function<void(std::string, void *)> devCheckPermisionCallback;

  /**
   * @brief This callback function is triggered when a plug-in
   *        event of a device is failed. Register by setDevConnectFailedCallback
   * function
   * @param [in] DevConnectFailed   	The Reason of devConnect Failed
   *
   * @param [in] string        		The SN of the current device is a
   * 14-digit string used to uniquely identify the device.
   * @param [in] void *        		Custom parameters passed in by the user
   *
   */
  typedef std::function<void(DevConnectFailed, std::string, Device::DevMode,
                             void *)>
  ...
  typedef std::function<void(const DeviceInfo &, void *)> devSnInfoCallback;
  ...
  void setDevCheckPermission(devCheckPermisionCallback callback, void *param);

  /**
   * @brief  Register device ConnectFailed event callback function.
   * @param  [in] callback   Callback function to be registered.
   * @param  [in] param      User-defined parameter
   */
  void setDevConnectFailedCallback(devConnectFailedCallback callback,
                                   void *param);
  /**
  ...
  getNeedCheckPermissionDevBySn(const std::string &dev_sn);

  void deleteDevBySn(const std::string &dev_sn);

  void askCheckResult(bool pass, const std::string &dev_sn, uint8_t user_type,
